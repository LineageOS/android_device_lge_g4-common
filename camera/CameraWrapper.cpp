...
static int camera_initialize(const camera3_device_t *device, const camera3_callback_ops_t *callback_ops)
{
...
static int camera_configure_streams(const camera3_device *device, camera3_stream_configuration_t *stream_list)
{
    ALOGV("%s->%08X->%08X", __FUNCTION__, (uintptr_t)device,
        (uintptr_t)(((wrapper_camera_device_t*)device)->vendor));
...
static int camera_register_stream_buffers(const camera3_device *device, const camera3_stream_buffer_set_t *buffer_set)
{
    ALOGV("%s->%08X->%08X", __FUNCTION__, (uintptr_t)device,
        (uintptr_t)(((wrapper_camera_device_t*)device)->vendor));
...
static const camera_metadata_t *camera_construct_default_request_settings(const camera3_device_t *device, int type)
{
    ALOGV("%s->%08X->%08X", __FUNCTION__, (uintptr_t)device,
        (uintptr_t)(((wrapper_camera_device_t*)device)->vendor));
...
static int camera_process_capture_request(const camera3_device_t *device, camera3_capture_request_t *request)
{
    ALOGV("%s->%08X->%08X", __FUNCTION__, (uintptr_t)device,
        (uintptr_t)(((wrapper_camera_device_t*)device)->vendor));
...
static void camera_get_metadata_vendor_tag_ops(const camera3_device *device, vendor_tag_query_ops_t* ops)
{
    ALOGV("%s->%08X->%08X", __FUNCTION__, (uintptr_t)device,
        (uintptr_t)(((wrapper_camera_device_t*)device)->vendor));
...
static void camera_dump(const camera3_device_t *device, int fd)
{
    ALOGV("%s->%08X->%08X", __FUNCTION__, (uintptr_t)device,
        (uintptr_t)(((wrapper_camera_device_t*)device)->vendor));
...
static int camera_flush(const camera3_device_t* device)
{
    ALOGV("%s->%08X->%08X", __FUNCTION__, (uintptr_t)device,
        (uintptr_t)(((wrapper_camera_device_t*)device)->vendor));
        ...
        hw_device_t **device)
{
    ...
    int num_cameras = 0;
    int cameraid;
    wrapper_camera_device_t *camera3_device = NULL;
    ...
    if (name != NULL) {
        ...
        camera3_device = (wrapper_camera_device_t*)malloc(sizeof(*camera3_device));
        if (!camera3_device) {
        ...
        camera3_device->base.common.close = camera_device_close;
        camera3_device->base.ops = camera3_ops;

        camera3_ops->initialize = camera_initialize;
        camera3_ops->configure_streams = camera_configure_streams;
        camera3_ops->register_stream_buffers = NULL; /* camera_register_stream_buffers; // deprecated */
        camera3_ops->construct_default_request_settings = camera_construct_default_request_settings;
        camera3_ops->process_capture_request = camera_process_capture_request;
        camera3_ops->get_metadata_vendor_tag_ops = camera_get_metadata_vendor_tag_ops;
        camera3_ops->dump = camera_dump;
        camera3_ops->flush = camera_flush;
